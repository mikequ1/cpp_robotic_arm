...
void Arm::controldirect() {
    std::array<double, 16> initial_pose;
    double time = 0.0;

    vector<array<double, 16>> pp; //Previous Poses
    vector<array<double, 16>> dp; //Change in Poses
    
    m_robot->control([&time, &initial_pose, &pp, &dp](const franka::RobotState& robot_state,
                                         franka::Duration period) -> franka::CartesianPose {
        time += period.toSec();

        if (time == 0.0) {
            initial_pose = robot_state.O_T_EE_c;
            array<double, 16> empty_array = {};
            empty_array.fill(0);
            dp.push_back(empty_array);
        }

        if (pp.size() < 20) {
            array<double, 16> dp_entry;
            if (pp.size() != 0) {
                for (int i = 0; i < 16; i++) 
                    dp_entry[i] = robot_state.O_T_EE_c[i] - pp[pp.size()-1][i];
            }
            dp.push_back(dp_entry);
            pp.push_back(robot_state.O_T_EE_c);
        } else {
            array<double, 16> dp_entry;
            pp.erase(pp.begin());
            dp.erase(dp.begin());
        ...
        if (time <= 1) {
            ...
            for (int i = 0; i < 16; i++) {
                new_pose[i] = robot_state.O_T_EE_c[i] + dp[dp.size()-1][i]/1.5;
                cout << new_pose[i] << " ";
            }
            cout << endl << endl;
        }
