...
static const double V_FACTOR = 0.06; // in m/s
...
void getEE(std::array<double, 16>& ee_pose, std::array<double, 4>& nextpose, std::array<double, 2>& nextObj, int sock, int bs, int axis_x, int axis_y) {
  std::string state = "s,";
  for (int i = 0; i < 16; i++) {
  ...
  state.append(",");
  state.append(std::to_string(axis_x));
  state.append(",");
  state.append(std::to_string(axis_y));
  state.append(",");
...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    if (rst) {
      ...
      robot.absolute_cart_motion(0.3, -0.023, 0, 3);
    ...
    robot.get_franka_robot().control([=, &time, &state, &time_goal, &time_state, 
      ...
      if (state == 1) {
        ...
        array<double, 4> next;
        getEE(pose, next, next_obj, sock, bs, axis_x, axis_y);
        for (int i = 0; i < 3; i++) {
          next[i] += cur[i];
        }
        q.push(next);
      ...
      if (state == 3) {
        ...
        if (time>3.0 && (reached_object(cur, next_obj, 0.007)&& cur[2] < -0.03) && gripper_state == -1) {
          cout << "REACHED OBJECT for grasp" << endl;
          state = 4;
          time_state = 0;
          gripper_state = 2;
        } else if (time >3.0 && (reached_object(cur, next_obj, 0.016) && cur[2] < -0.03) && gripper_state == 1){
    ...
    if (gripper_state == 2) {
      robot.absolute_cart_motion(pose[12],pose[13],-0.065,1);
      if (robot.get_franka_gripper().grasp(0.02, 0.025, 10, 0.01, 0.01)){
        gripper_state = 1;
        ...
        robot.get_franka_gripper().move(gripper_read.max_width, 0.025);
      }
      robot.absolute_cart_motion(pose[12],pose[13],-0.04,1);
    } else if (gripper_state == 3) {
      robot.absolute_cart_motion(pose[12],pose[13],-0.06,1);
      franka::GripperState gripper_read = robot.get_franka_gripper().readOnce();
      if (robot.get_franka_gripper().move(gripper_read.max_width, 0.025)){
        gripper_state = -1;
